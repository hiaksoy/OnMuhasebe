...
public class CreateMakbuzDtoValidator : AbstractValidator<CreateMakbuzDto>
{
    ...
    {
        RuleFor(x => x.MakbuzTuru).IsInEnum().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["ReceiptType"]])
                                         .NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["ReceiptType"]]);
