...
public class FaturaHareketDtoValidator : AbstractValidator<FaturaHareketDto>
{
    public FaturaHareketDtoValidator(IStringLocalizer localizer)
    {
        ...
        RuleFor(x => x.HareketTuru).IsInEnum().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["RowType"]])
                                   .NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["RowType"]]);

        RuleFor(x => x.StokId).NotEmpty().When(x=>x.HareketTuru == FaturaHareketTuru.Stok).WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["Stock"]]);

        RuleFor(x => x.HizmetId).NotEmpty().When(x => x.HareketTuru == FaturaHareketTuru.Hizmet).WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["Service"]]);

        RuleFor(x => x.MasrafId).NotEmpty().When(x => x.HareketTuru == FaturaHareketTuru.Masraf).WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["Expense"]]);

        RuleFor(x => x.DepoId).NotEmpty().When(x => x.HareketTuru == FaturaHareketTuru.Stok).WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["Warehouse"]]);

        RuleFor(x => x.Miktar).NotNull().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["Quantity"]])
           .GreaterThanOrEqualTo(0).WithMessage(localizer[OnMuhasebeDomainErrorCodes.GreaterThenOrEqual, localizer["Quantity"], localizer["ToZero"], localizer["ThanZero"]]);
