...
public class CreateCariDtoValidator : AbstractValidator<CreateCariDto>
{
    public CreateCariDtoValidator(IStringLocalizer<OnMuhasebeResource> localizer)
    {
        ...
        RuleFor(x=> x.VergiDairesi).MaximumLength(CariConsts.MaxVergiDairesiLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["TaxOffice"], CariConsts.MaxVergiDairesiLength]);
      
        RuleFor(x=> x.VergiNo).MaximumLength(CariConsts.MaxVergiNoLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["TaxNumber"], CariConsts.MaxVergiNoLength]);
       
        RuleFor(x=> x.Telefon).MaximumLength(EntityConsts.MaxTelefonLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["PhoneNumber"], EntityConsts.MaxTelefonLength]);

        RuleFor(x=> x.Adres).MaximumLength(EntityConsts.MaxAdresLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["Address"], EntityConsts.MaxAdresLength]);
