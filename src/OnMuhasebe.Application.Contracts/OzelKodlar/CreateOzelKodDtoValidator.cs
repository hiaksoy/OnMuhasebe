...
public class CreateOzelKodDtoValidator : AbstractValidator<CreateOzelKodDto>
{
    public CreateOzelKodDtoValidator(IStringLocalizer<OnMuhasebeResource> localizer)
    {
        ...
        RuleFor(x => x.KodTuru).IsInEnum().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["CodeType"]])
                                            .NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["CodeType"]]);

        RuleFor(x => x.KartTuru).IsInEnum().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["CardType"]])
                                      .NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["CardType"]]);
