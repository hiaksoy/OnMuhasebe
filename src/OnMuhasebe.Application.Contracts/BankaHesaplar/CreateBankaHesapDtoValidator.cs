...
public class CreateBankaHesapDtoValidator : AbstractValidator<CreateBankaHesapDto>
{
    public CreateBankaHesapDtoValidator(IStringLocalizer<OnMuhasebeResource> localizer)
    {
        ...
        RuleFor(x => x.HesapTuru).IsInEnum().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["AccountType"]])
                                            .NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["AccountType"]]);

        RuleFor(x => x.BankaSubeId).Must(x => x.HasValue && x.Value != Guid.Empty).WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["BankBranch"]]);

        RuleFor(x=> x.HesapNo).NotEmpty().WithMessage(localizer[OnMuhasebeDomainErrorCodes.Required, localizer["AccountNumber"]])
                              .MaximumLength(BankaHesapConsts.MaxHesapNoLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["AccountNumber"], BankaHesapConsts.MaxHesapNoLength]);

        RuleFor(x => x.IbanNo).MaximumLength(BankaHesapConsts.MaxIbanNoLength).WithMessage(localizer[OnMuhasebeDomainErrorCodes.MaxLength, localizer["IbanNumber"], BankaHesapConsts.MaxIbanNoLength]); 
