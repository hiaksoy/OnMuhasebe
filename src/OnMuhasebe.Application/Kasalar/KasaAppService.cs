...
public class KasaAppService : OnMuhasebeAppService, IKasaAppService
{
    ...
    {
        var entities = await _kasaRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.SubeId == input.SubeId && x.Durum == input.Durum, x => x.Kod, x => x.OzelKod1, x => x.OzelKod2, x => x.MakbuzHareketler);
        var totalCount = await _kasaRepository.CountAsync(x => x.SubeId == input.SubeId && x.Durum == input.Durum);
        var mappedDtos = ObjectMapper.Map<List<Kasa>, List<ListKasaDto>>(entities);
