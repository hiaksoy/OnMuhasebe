...
public class MakbuzAppService : OnMuhasebeAppService, IMakbuzAppService
{
    ...
    {
        return await _makbuzRepository.GetCodeAsync(x => x.MakbuzNo, x => x.SubeId == input.SubeId && x.DonemId == input.DonemId && x.MakbuzTuru == input.MakbuzTuru && x.Durum == input.Durum);
    }

    public virtual async Task<PagedResultDto<ListMakbuzDto>> GetListAsync(MakbuzListParameterDto input)
    {
        var entities = await _makbuzRepository.GetPagedListAsync(input.SkipCount, input.MaxResultCount, x => x.MakbuzTuru == input.MakbuzTuru && x.SubeId == input.SubeId && x.DonemId == input.DonemId && x.Durum == input.Durum,
            x => x.MakbuzNo,
            x => x.Cari,
            x => x.Kasa,
            x => x.BankaHesap,
    ...
    public virtual async Task<SelectMakbuzDto> UpdateAsync(Guid id, UpdateMakbuzDto input)
    {
       ...
       await _makbuzManager.CheckUpdateAsync(id, input.MakbuzNo, entity, input.CariId, input.KasaId, input.BankaHesapId, input.OzelKod1Id, input.OzelKod2Id);
        ...
        await _makbuzRepository.UpdateAsync(mappedEntity);
        return ObjectMapper.Map<Makbuz, SelectMakbuzDto>(mappedEntity);
    }
}
