...
namespace OnMuhasebe.Entities.BankaSubeler;
public class BankaSubeManager : DomainService
{
    ...
    {
        await _bankaSubeRepository.RelationalEntityAnyAsync(x => x.BankaHesaplar.Any(y => y.BankaSubeId == id || x.MakbuzHareketler.Any(y => y.CekBankaSubeId == id)));
    }
}
