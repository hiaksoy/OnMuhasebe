    ...
    where TGetCodeInput : class, IEntityDto, IDurum, new()
{
    ...
    protected virtual async Task DeleteAsync()
    {
        ...
        await BaseService.ConfirmMessage(L["DeleteConfirmMessage"],
            async () =>
            {
    ...
    protected virtual async Task OnSubmit()
    {
        ...
        {
            var createInput = ObjectMapper.Map<TGetOutputDto, TCreateInput>(BaseService.DataSource);
        ...
        {
            var updateInput = ObjectMapper.Map<TGetOutputDto, TUpdateInput>(BaseService.DataSource);
        ...
        var savedEntityIndex = BaseService.ListDataSource.FindIndex(x => x.Id == BaseService.DataSource.Id);
        ...
        if (savedEntityIndex > -1)
            BaseService.SelectedItem = BaseService.ListDataSource.SetSelectedItem(savedEntityIndex);

        else
        {
            BaseService.SelectedItem = BaseService.ListDataSource.GetEntityById(BaseService.DataSource.Id);
        }
    }
}
